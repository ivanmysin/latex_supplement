...
\begin{eqnarray}
C\frac{dV_s}{dt}=-I_L-I_{Na}-I_{kdr}-I_{CaN}I_{CaL}-I_{CaN} -I_{syn} + I_{ext}-I_A-I_M-I_h-I_{sAHP}-I_{mAHP}- \nonumber \\ -I_{CaT}-I_{CaR}-I_{buff}
\end{eqnarray}

Уравнение баланса для соматического компартмента:
\begin{eqnarray}
C\frac{dV_s}{dt}=-I_L-I_{Na}-I_{kdr}-I_{CavL}--I_{CavN} \nonumber \\ -I_{HCN}-I_{HCNolm}-I_{KCaS}-I_{Kdrfast}-I_{Kdrngf}-I_{Kdrslow}-I_{KvA}-I_{KvAngf}
-I_{KvAolm}-I_{KvCaB}-I_{KvGroup}-I_{KvM}-I_{Navcck}-I_{Navngf}
-I_{syn} + I_{ext}
\end{eqnarray}
