...
C\frac{dV}{dt}=-\sum{I_{ch}}-I_{syn} + I_{ext}
\end{eqnarray}
where C is capacity in $\mu F/ cm^2$, V is potetial in mV, t is time in ms, I is current in $\mu A/cm^2$. $I_{ch}$ are transmembrane currents, they are described in following sections, and tables with parameters are given in \ref{tables_of_neurons}. Common equation for transmemrane currents is
\begin{equation}
I_{ch}= g_{max, ch}\cdot x_1^N\cdot x_2^M \cdot x_3^K \cdot (V-E_{ch})
\end{equation}
where $ g_{max, ch}$ maximal coduntance in $mS/ cm^2$, $E_{ch}$ is reversal potantial for this channel in $mV$. $x_1,\ x_2, \ x_3$ are gate variables. All compartments have leak current:
\begin{equation}
...
\end{equation}
where $I_{ext, mean}$ is constant value,  $ \eta$ is random value from normal disribution with nean equals 0 and standart deviation equals 1, $\sigma_{ext}=0.005 \mu A/cm^2/\sqrt{ms}$ is std of noise.
...
\section{Synapses models} \label{synapse_models}
\section{Simulation of field potetial} \label{field_potetial_model}
